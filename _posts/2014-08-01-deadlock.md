...
另个两个堆栈中都出现了`SHELL32!SHGetFolderPathW`，是否这个函数执行时需要占用一个临界区呢？`1e84`线程中比较靠近栈顶的`SHELL32!kfapi::CCriticalSectionLock::CCriticalSectionLock`这一帧应该就能给出肯定的答案了。
...
2. 添加对LoadLibrary的HOOK，拒绝TaobaoProtectSE.dll注入。已验证可生效。  
