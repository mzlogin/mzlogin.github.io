...
1. 去 <http://www.sbcl.org/> 下载安装 sbcl 的最新版。
	...
	后来经验证，发现实际上是 sbcl 在安装过程中写的环境变量没有生效导致的，sbcl 在安装过程中会设置两个环境变量，新建一个 `SBCL_HOME = sbcl 安装路径`，在 PATH 中添加 sbcl 安装路径。这两个变量必须是生效的，不然到安装目录下 CMD 运行 sbcl 会提示

	```
	can't find core file at C:Program Files (x86)/sbcl/lib/sbcl//sbcl.core
	```

	而此时在 Emacs 中运行 slime 会提示
